...
PROJECT_ID <- "2024-07-KKV" # REPLACE THIS EXAMPLE VALUE 
...
survey_id <- all_surveys %>% 
  filter(name == "CorrelAid Projekt 2024-07-KKV") %>% 
  pull(uid)
...
project_roles_df <- applications %>% 
  dplyr::select(applicant_id, project_role = `project_role/project_role_header_001`) %>% 
  dplyr::mutate(project_id = "2024-07-KKV") %>% 
